...
// 않아 모서리를 뚫고 나온다. 직접 그리는 커스텀 오버레이로 대체하되, 썸 길이는 네이티브처럼
// 콘텐츠 비율로 계산하고 최소 길이를 둔다.
...
// 모달 스크롤바와 PageScrollbar(페이지 전체 스크롤)가 공유하는 콘텐츠 비율 기반 계산.
// trackHeight는 시각적 트랙 길이(헤더/푸터를 피해 clientHeight보다 짧을 수 있음) — 진행률
// (scrollProgress)은 항상 실제 clientHeight 기준이어야 스크롤 끝에서 썸도 트랙 끝까지 도달한다.
